...
def train(rank, world_size, args):
    ...
    optimizer = optim.AdamW([
        {'params': ssm_params, 'lr': 5e-4,'weight_decay':4e-2},
        {'params': feed_params, 'lr': 5e-4,'weight_decay':4e-2},
        {'params': other_params, 'lr': 5e-4,'weight_decay':4e-2},
    ])
    ...
    for epoch in range(epoch_0,epochs):
        ...
        if rank == n_ka:
            ...
            # Save model
            save_path = os.path.join('../../model/pathfinder', f'S4_s4d_epoch_{epoch + 1}.pth')
            torch.save(model.module.state_dict(), save_path)
            loss_path = os.path.join('../../model/pathfinder', f'S4_s4d_loss.pth')
