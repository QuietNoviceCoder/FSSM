...
def train(rank, world_size, args):
    ...
    if rank == n_ka:
        run = wandb.init(
            project="path-finderX",
            name = "4-s4d+2-fs4d,100epochs",
            config={
                "learning_rate": 5e-4,
    ...
    # Load data
    image = torch.load('../../data/pathfinder/pathfinder128.pt',weights_only=False)
    ...
    for epoch in range(epoch_0,epochs):
        ...
        if rank == n_ka:
            ...
            # Save model
            save_path = os.path.join('../../model/pathfinderX', f'S4_fs4drel_epoch_{epoch + 1}.pth')
            torch.save(model.module.state_dict(), save_path)
            loss_path = os.path.join('../../model/pathfinderX', f'S4_fs4drel_loss.pth')
