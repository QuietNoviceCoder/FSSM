...
class model(nn.Module):
    def __init__(self):
        ...
        # self.ssm2 = sf.SSM_Block(hidden_size, step, activation, text_len, emb_dim, dropout=0,norm='BN')
...
optimizer = optim.AdamW([
    {'params': ssm_params, 'lr': 0.001*1.5},
    {'params': other_params, 'lr': 0.01*1.5},
    ],weight_decay=0.05)
...
for epoch in range(epochs):
    ...
    for batch_idx,train in pbar:
        ...
        pbar.set_postfix({
            ...
            "Batch": f"{batch_idx + 1}/{current_train_samples/batch_size}"  # 当前 batch 进度
        })
    scheduler.step()
    ...
    save_path = os.path.join('../../model/text', f'epoch_fc2_{epoch + 1}.pth')
    torch.save(model.state_dict(), save_path)
    if test_acc > best_acc and epoch > 1:
        best_acc = test_acc
        save_path = os.path.join('../../model/text', f'epoch_fc2_{epoch + 1}_best.pth')
        torch.save(model.state_dict(), save_path)
    print(f"Epoch [{epoch + 1}] "
          f"Train Loss: {train_loss:.4f} Acc: {train_acc:.4f} | "
          f"test Loss: {test_loss:.4f} Acc: {test_acc:.4f}")
save_path = os.path.join('../../model/text', f'epoch_fc2_{epochs}.pth')
loss_path = os.path.join('../../model/text', f'fc2_loss.pth')
torch.save(model.state_dict(), save_path)
