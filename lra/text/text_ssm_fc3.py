...
train_loader = DataLoader(Idmb['train'], batch_size, shuffle=True)
...
class model(nn.Module):
    def __init__(self):
        ...
        self.layer1 = sf.SSM_Block(hidden_size, step, activation, text_len, emb_dim, dropout=0, norm='BN')
        self.layer2 = sf.SSM_Block(hidden_size, step, activation, text_len, emb_dim, dropout=0, skip=True, norm='BN')
        self.layer3 = sf.SSM_Block(hidden_size, step, activation, text_len, emb_dim, dropout=0, skip=True, norm='BN')
        self.layer4 = sf.SSM_Block(hidden_size, step, activation, text_len, emb_dim, dropout=0, skip=True, norm='BN')
    ...
    def forward(self,x):
        ...
        return logits
model = model()
# model.load_state_dict(torch.load('../model/epoch_fc0_4.pth'))
#设置优化器
model.to(device)
...
save_path = os.path.join('../../model/text', f'epoch_fc3_{epochs}.pth')
loss_path = os.path.join('../../model/text', f'fc3_loss.pth')
torch.save(model.state_dict(), save_path)
