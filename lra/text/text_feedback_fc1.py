...
class model(nn.Module):
    ...
    def forward(self,x):
        ...
        return logits,H1
model = model()
# model.load_state_dict(torch.load('../../model/text/epoch_fc1_4.pth'))
#设置优化器
model.to(device)
...
for epoch in range(epochs):
    ...
    for batch_idx,train in pbar:
        ...
        pbar.set_postfix({
            ...
            "Batch": f"{batch_idx + 1}/{5000/batch_size}"  # 当前 batch 进度
        })
    scheduler.step()
    ...
    if (epoch+1) % 2 == 0:
        save_path = os.path.join('../../model/text', f'epoch_fc1_{epoch + 1}.pth')
        torch.save(model.state_dict(), save_path)
    print(f"Epoch [{epoch + 1}] "
          f"Train Loss: {train_loss:.4f} Acc: {train_acc:.4f} | "
          f"test Loss: {test_loss:.4f} Acc: {test_acc:.4f}")
save_path = os.path.join('../../model/text', f'epoch_fc1_{epochs}.pth')
loss_path = os.path.join('../../model/text', f'fc1_loss.pth')
torch.save(model.state_dict(), save_path)
