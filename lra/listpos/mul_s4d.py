...
class Model(nn.Module):
    def __init__(self, batch_size, len, emb_dim, hidden_size, step, activation):
        ...
        self.layer1 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
        self.layer2 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
        self.layer3 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
        self.layer4 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
        self.layer5 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
        self.layer6 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
        self.layer7 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
        self.layer8 = sf.S4D_Block(hidden_size, activation, emb_dim, dropout=0.1, skip=True, norm='BN')
...
def train(rank, world_size, args):
    ...
    optimizer = optim.AdamW([
        {'params': ssm_params, 'lr': 5e-3,'weight_decay':5e-2},
        {'params': feed_params, 'lr': 5e-3,'weight_decay':5e-2},
        {'params': other_params, 'lr': 5e-3,'weight_decay':5e-2},
    ])
    ...
    for epoch in range(epoch_0,epochs):
        ...
        if rank == 0:
            ...
            print(f"Epoch [{epoch + 1}] "
                  ...
                  f"lr: {current_lr:.9f}")
            # Save model
            save_path = os.path.join('../../model/listops', f'S4_s4d_epoch_{epoch + 1}.pth')
            torch.save(model.module.state_dict(), save_path)
            loss_path = os.path.join('../../model/listops', f'S4_s4d_loss.pth')
