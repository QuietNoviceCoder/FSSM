...
class Model(nn.Module):
    def __init__(self, batch_size, len, emb_dim, hidden_size, step, activation):
        ...
        self.layer5 = fssm.FS4Ddeq_model(hidden_size, activation, emb_dim, layers=4,
                                         final_act='gelu', skip=True, norm='BN', dropout=0.0,
                                         state_size=[batch_size, len, emb_dim],
                                         feed_size=[batch_size, len, emb_dim],
                                         feed_act='gelu',
                                         )
        self.layer6 = fssm.FS4Ddeq_model(hidden_size, activation, emb_dim, layers=2,
                                         final_act='gelu', skip=True, norm='BN', dropout=0.0,
                                         state_size=[batch_size, len, emb_dim],
                                         feed_size=[batch_size, len, emb_dim],
                                         feed_act='gelu',
    ...
    def forward(self, x):
        ...
        y1, loss1, n_steps1, zhanbi1 = self.layer6(r)
        y1 = self.layer1(y1)
        y2 = self.layer2(y1)
        y3 = self.layer3(y2)
        y4 = self.layer4(y3)
...
def train(rank, world_size, args):
    ...
    optimizer = optim.AdamW([
        ...
        {'params': feed_params, 'lr': 1e-3,'weight_decay':1e-2},
