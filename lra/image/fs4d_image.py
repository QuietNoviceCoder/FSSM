...
class Model(nn.Module):
    ...
    def forward(self,x):
        ...
        y1, loss, n_steps, zhanbi = self.layer5(r)
...
def train(rank, world_size, args):
    ...
    model.load_state_dict(torch.load('../../model/image/S4_fs4d_epoch_10.pth'),strict=False)
    model = model.to(rank)
    ...
    optimizer = optim.AdamW([
        ...
        {'params': feed_params, 'lr': 1e-4,'weight_decay':5e-2},
    ...
    for epoch in range(epoch_0,epochs):
        ...
        for batch_idx, (batch_x, batch_y) in pbar:
            ...
            loss = criterion(logits, batch_y) + 5 * jac_loss
            # Backward pass
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
        ...
        if rank == n_ka:
            ...
            if epoch % 5 == 0:
                checkpoint = {
                    ...
                    "scheduler_state_dict": reduce_scheduler.state_dict(),
                    "loss": loss,
                }
                torch.save(checkpoint, f'../../model/image/checkpoint{epoch}.pth')
            if val_acc > best_acc:
                best_acc = val_acc
